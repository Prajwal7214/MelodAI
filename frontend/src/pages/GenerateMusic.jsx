...
import { Loader2, Disc3, Music, Sparkles, ArrowRight } from 'lucide-react';
...
const GenerateMusic = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (tracks.length > 0) {
        ...
        const playableSong = tracks.find(s => s.preview_url) || tracks[0];
        if (playableSong) playSong(playableSong, tracks);
  ...
  return (
    <div className="relative min-h-screen bg-[#F5F5F7] text-zinc-900 pt-16 pb-24 px-6 transition-colors duration-500 overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-[400px] pointer-events-none opacity-40">
        <Antigravity
          ...
          waveAmplitude={1} particleSize={1.5} lerpSpeed={0.05} color="#000000"
          autoAnimate particleVariance={1} rotationSpeed={0} depthFactor={1}
      ...
      <div className="max-w-6xl mx-auto relative z-10 mt-12">
        ...
        {isGuest && !isLoggedIn && (
          <motion.div
            ...
            className="flex items-center justify-between bg-violet-500/10 border border-violet-500/20 rounded-xl px-4 py-3 mb-8"
          >
            <div className="flex items-center gap-2.5">
              <Sparkles className="w-4 h-4 text-violet-500 shrink-0" />
              <span className="text-sm text-violet-700 font-medium">
                You're on <strong>free preview</strong> — Studio access only.
              </span>
            </div>
            <Link to="/signup">
              <motion.button
                ...
                whileTap={{ scale: 0.97 }}
                className="flex items-center gap-1.5 text-xs font-bold text-white bg-gradient-to-r from-violet-600 to-pink-500 px-3 py-1.5 rounded-lg shadow-sm"
              >
              ...
              >
                ...
                <div className="flex items-center gap-3 mb-4">
                  <button
                    ...
                    className="flex-shrink-0 w-10 h-10 flex items-center justify-center bg-violet-600 shadow-md shadow-violet-500/20 rounded-full hover:bg-violet-500 transition-colors"
                  >
