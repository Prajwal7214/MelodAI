...
import { Play, Pause, Trash2, Calendar, Music4, Target, Zap, Leaf, Play as PlayIcon } from 'lucide-react';
...
const MusicHistory = () => {
  ...
  const handlePlaySession = (e, session) => {
    e.stopPropagation();
  ...
  const handleSongPlay = (e, song, sessionSongs) => {
    e.stopPropagation();
    playSong(song, sessionSongs);
  };

  const isSessionPlaying = (session) => {
    return isPlaying && session.songs.some(s => s.id === currentSong?.id);
  };

  return (
    <div className="relative min-h-screen bg-black text-white pt-28 pb-24 px-6">
                  ...
                  <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                    ...
                    <div className="flex-1 space-y-4">
                      ...
                      <div className="flex gap-2 pt-2">
                        {session.songs.slice(0, 4).map((s, i) => (
                          <div key={i} className="w-12 h-12 md:w-16 md:h-16 rounded-lg overflow-hidden bg-zinc-800 shrink-0">
                            {s.artwork ? (
                              <img
                                src={s.artwork}
                                alt="cover"
                        ...
                        {session.songs.length > 4 && (
                          <div className="w-12 h-12 md:w-16 md:h-16 rounded-lg bg-zinc-800 flex items-center justify-center border border-zinc-700 text-xs text-zinc-400 font-semibold shrink-0">
                            +{session.songs.length - 4}
                          </div>
                        )}
                      </div>
                    ...
                    <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-start gap-4">
                      <div className="flex gap-2 shrink-0 order-2 md:order-1">
                        <button
                          onClick={(e) => handlePlaySession(e, session)}
                          className="flex items-center gap-2 rounded-full px-4 py-1.5 text-sm font-medium bg-gradient-to-r from-violet-600 to-pink-500 text-white hover:opacity-90 transition-opacity"
                        >
                        ...
                        <button
                          ...
                          className="w-8 h-8 flex items-center justify-center rounded-full border border-zinc-800 text-zinc-500 hover:border-red-500/30 hover:text-red-400 transition-colors"
                        >
                          ...
                          {session.songs.map((song) => (
                            <div
                              ...
                              className={`flex items-center gap-4 p-2.5 rounded-xl transition-colors ${
                                currentSong?.id === song.id ? 'bg-violet-500/10 border-l-2 border-violet-500' : 'hover:bg-zinc-800/50 border-l-2 border-transparent'
                              }`}
                            >
                              <div className="w-10 h-10 rounded overflow-hidden bg-zinc-800 shrink-0">
                                {song.artwork ? (
                                  ...
                                  <div className="w-full h-full flex items-center justify-center">
                                    <Music4 className="w-4 h-4 text-zinc-600" />
                                  </div>
                                )}
                              </div>
                              ...
                              <div className="text-xs font-mono text-zinc-600 mr-4 shrink-0">
                                {song.duration}
                              </div>
                              <PlayerHeartButton song={song} onToast={showToast} />
                              <button
                                className="w-8 h-8 rounded-full bg-zinc-800 hover:bg-violet-600 flex items-center justify-center transition-colors shrink-0 ml-2"
                                onClick={(e) => handleSongPlay(e, song, session.songs)}
                              >
